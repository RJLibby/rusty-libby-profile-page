...
- ssh into the server
    - For Mac and Linux: `ssh root@rustylibby.pro`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo rlibby`
    - replace rlibby with your own
- create password
    - `passwd rlibby`
- Move .ssh key file
    - `cp -r .ssh /home/rlibby`
- change ownership of the .ssh directory
    - `cd /home/rlibby`
    - `chown -R rlibby:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh rlibby@rustylibby.pro`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- create a directory for your deployment
    - `sudo mkdir -p /var/www/rustylibby.pro/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/rustylibby.pro/html`
...
- `sudo vim /etc/nginx/sites-available/rustylibby.pro`
...
server {
    ...
    root /var/www/rustylibby.pro/html;
    index index.html index.htm index.nginx-debian.html;

    server_name rustylibby.pro www.rustylibby.pro;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/rustylibby.pro /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/rlibby/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/rlibby/.ssh/github_deploy_key.pub >> /home/rlibby/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/rlibby/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H rustylibby.pro >> ~/.ssh/known_hosts 
         scp -r ./dist/* rlibby@rustylibby.pro:/var/www/rustylibby.pro/html
```
